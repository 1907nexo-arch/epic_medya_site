...
function triggerFluidTransition(onCompleteCallback) {
    try {
        ...
        onCompleteCallback(); // Herhangi bir hata durumunda kilitlenmemesi için geçişi doğrudan gerçekleştir
    }
}
        ...
        link.addEventListener('click', (e) => {
            ...
            if (href && !href.startsWith('#') && href !== 'javascript:void(0);') {
                ...
                if (link.classList.contains('logo-trigger')) {
                    clearTimeout(redirectTimeout); // Logo durumunda dışa yönlendirmeyi iptal et
                    triggerFluidTransition(() => {
                        openCinemaReel();
                    });
                } 
                else if (link.classList.contains('about-trigger')) {
                    clearTimeout(redirectTimeout); // Hakkımızda panel açılışında dışa yönlendirmeyi iptal et
                    const menuOverlay = document.querySelector('.mobile-menu-overlay');
